...
UPlot.uplot(dataset, gens, fits, runs, param_types, 'big_plot1') # fitness

deviation_distances = []
perms.each do |perm|
	deviation_distances << PDist.deviation(original_order, perm)
end
UPlot.uplot(dataset, gens, deviation_distances, runs, param_types, 'big_plot_dev')

square = []
perms.each do |perm|
	square << PDist.square(original_order, perm)
end
UPlot.uplot(dataset, gens, square, runs, param_types, 'big_plot_square')

hamming = []
perms.each do |perm|
	hamming << PDist.hamming(original_order, perm)
end
UPlot.uplot(dataset, gens, hamming, runs, param_types, 'big_plot_hamming')

rdist = []
perms.each do |perm|
	rdist << PDist.rdist(original_order, perm)
end
UPlot.uplot(dataset, gens, rdist, runs, param_types, 'big_plot_r')

lcs = []
perms.each do |perm|
	lcs << PDist.lcs(original_order, perm)
end
UPlot.uplot(dataset, gens, lcs, runs, param_types, 'big_plot_lcs')

kt = []
perms.each do |perm|
	kt << PDist.kendalls_tau(original_order, perm)
end
UPlot.uplot(dataset, gens, kt, runs, param_types, 'big_plot_kendalls')