...
if data_plot == 'plot' || data_plot == 'both'
	MetricWork.metric_test_plot(dataset, filename, metric, metric, "The change in #{metric} of permuations, increasingly distant
		in the search space from the optimal arrangement.
		Permutations are of contigs from a genome.", filename)
end
