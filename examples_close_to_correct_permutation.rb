...
example_perms.each do |population|
	y = 1
	Dir.mkdir(File.join(Dir.home, "#{location}/#{dataset}/#{div/1000}K_mutant_perm_examples/gen_#{dirs[x]}"))
	population.each do |permutation|
		WriteIt::write_txt("#{Dir.home}/#{location}/#{dataset}/#{div/1000}K_mutant_perm_examples/gen_#{dirs[x]}/permutation#{y}", permutation)
		...
		SNPdist::plot_hyp2(perm_ratio, location, "#{dataset}/#{div/1000}K_mutant_perm_examples", dirs[x], genome_length, "permutation#{y}")
		y+=1
	end
	x+=1
end
